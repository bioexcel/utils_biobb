...
def main():
    ...
    parser.add_argument("--template", default=TEMPL,
                        help="Template for XML galaxy adapter")
    parser.add_argument("--xml_dir", default=XML_DIR,
                        help="Path to place output XML recipes")
    parser.add_argument(dest="base_biobb_dir",
                        help="Json schema from building blocks group")
    parser.add_argument("--file_types", action="store_true",
                        help="List required File Types on log")
    parser.add_argument("--tools_xml", action="store",
                        help="Generate tools panel XML")
    ...
    for group in os.listdir(args.base_biobb_dir):
        ...
        if write_conf:
            tools_xml.write(
                f"<section id=\"{group}\" name=\"{SECTIONS[group]}\" >\n")

        # Parsing json schemas
        group_schema = opj(args.base_biobb_dir, group, group,
                           'json_schemas', group + '.json')
        try:
        ...
        for block in group_data['blocks']:
            ...
            for f in schema_data['properties']:
                if f != 'properties':
                    ...
                    tool_data['label'] = schema_data['properties'][f]['filetype'] + \
                        ' ' + tool_data['format'].upper()
                    ...
                    for k, v in schema_data['properties'][f]['properties'].items():
                        ...
                        if v['type'] in ('string', 'select'):
                            if 'multiple' in v and v['multiple'] != 'false':
                                # ["${'","'.join($properties.k)}"]
                                txt = "__ob____dq__${'__dq__,__dq__'.join($properties." + \
                                    k + ")}__dq____cb__"
                            else:
                                # "${k}"
            ...
            if write_conf:
                tools_xml.write(
                    f"  <tool file=\"{SERVER_BASE_DIR}/{group}/biobb_{data['name']}.xml\" />\n")
            if args.file_types:
